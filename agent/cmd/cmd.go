...
import (
	...
	"github.com/lppgo/nova/utils/netutil"
...
func ParseFlags() *Flags {
	...
	// flag.IntVar(&flags.AgentRegistryPort, "agent-registry-port", 0, "port which agent registry listens on")
	flag.StringVar(
...
func Run(flags *Flags, opts ...Option) {
	...
	if overrides.logger != nil {
		...
		defer func() {
			logSyncErr := zlog.Sync()
			if logSyncErr != nil {
				log.Errorf("logSyncErr:%s\n", logSyncErr.Error())
			}
		}()
	}
	...
	// go func() {
	err = http.ListenAndServe(addr, agentServer.Handler())
	if err != nil {
		log.Errorf("agent server ListenAndServe Error :%s", err.Error())
	}
	log.Infof(" agent server [%s] is running ... ", addr)
	...
	// go heartbeat(stats)
	//
	// nginxRunErr := nginx.Run(config.Nginx, map[string]interface{}{
