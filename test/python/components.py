...
class Proxy(Component):
    ...
    def __init__(self, zone, origin_cluster, build_indexes):
        ...
        self.port = find_free_port()
        self.config_file = 'test-{zone}.yaml'.format(zone=zone)
        self.name = 'kraken-push-cli-{zone}'.format(zone=zone)
        ...
        self.volumes = create_volumes('push-cli', self.name)
    ...
    def new_container(self):
        return new_docker_container(
            name=self.name,
            image=dev_tag('kraken-push-cli'),
            ports={self.port: self.port},
            environment={},
            command=[
                '/usr/bin/kraken-push-cli',
                '--config=/etc/kraken/config/push-cli/{config}'.format(config=self.config_file),
                '--port={port}'.format(port=self.port),
            ],
            volumes=self.volumes,
...
class Cluster(object):
    ...
    def __init__(
        ...
        # TODO(codyg): Some tests rely on the fact that push-cli and agents point
